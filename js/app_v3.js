...
/* ── Guarda contexto para o choice screen ── */
let _colabChoiceContext = { nomeCard:'', turno:'', dia:'' };
...
function _iniciarFluxoColaborador() {
  ...
  if (turnoForce === 'dia' || turnoForce === 'noite') {
    ...
    _invalidarCache();
    _colabChoiceContext = { nomeCard, turno: turnoForce, dia: S.dia };
    _mostrarEscolhaColaborador(nomeCard, turnoForce);
  } else {
...
/* ── Exibe a tela de escolha: Executar Tarefas / Ranking ── */
function _mostrarEscolhaColaborador(nomeCard, turno) {
  const tela = document.getElementById('screen-colab-choice');
  if (!tela) {
    // Tela não existe no HTML — vai direto para o card (fallback seguro)
    selectColaborador(nomeCard);
    return;
  }

  // Preenche info do colaborador
  const elNome = document.getElementById('cc-name');
  const elSub  = document.getElementById('cc-sub');
  const elEmoji= document.getElementById('cc-emoji');
  const turnoLabel = turno === 'dia' ? '☀️ Turno Dia' : '🌙 Turno Noite';
  const nomeExibir  = JB_SESSION ? (JB_SESSION.nome || JB_SESSION.username || nomeCard) : nomeCard;
  if (elNome)  elNome.textContent  = nomeExibir;
  if (elSub)   elSub.textContent   = `${turnoLabel} — o que deseja fazer?`;
  // Emoji baseado no nome_card (reutiliza COLLAB_EMOJI)
  if (elEmoji) {
    const mapEmoji = (typeof COLLAB_EMOJI !== 'undefined') ? COLLAB_EMOJI : {};
    elEmoji.textContent = mapEmoji[(nomeCard||'').toUpperCase()] || '👷';
  }

  showScreen('screen-colab-choice');
}

/* ── Ação: Executar Tarefas ── */
function _colabChoiceExecutar() {
  const { nomeCard, turno, dia } = _colabChoiceContext;
  if (!nomeCard) { showScreen('screen-welcome'); return; }
  S.turno = turno;
  S.dia   = dia;
  _invalidarCache();
  selectColaborador(nomeCard);
...
/* ── Ação: Ver Ranking ── */
function _colabChoiceRanking() {
  const sess = JB_SESSION;
  // Determina departamento pelo nome_card
  const nomeCard = (sess?.nome_card || '').trim().toUpperCase();
  const COLLAB_DEPT_RANK = {
    'ANTONIEL':'ATENDIMENTO','ANTONIEL INICIO':'ATENDIMENTO','ANTONIEL FINALIZAÇÃO':'ATENDIMENTO',
    'MARCUS':'ATENDIMENTO','MARCUS INICIO':'ATENDIMENTO','MARCUS FINALIZAÇÃO':'ATENDIMENTO',
    'ALINE':'ATENDIMENTO','ALINE INICIO':'ATENDIMENTO','ALINE FINALIZAÇÃO':'ATENDIMENTO',
    'DESPACHO':'ATENDIMENTO','ATENDIMENTO / DESPACHO':'ATENDIMENTO','ABC':'ATENDIMENTO',
    'ALAN RICARDO':'OPERACAO','SAMUEL':'OPERACAO','LOHAINE':'OPERACAO',
    'GABRIEL LEITE':'OPERACAO','FRITADEIRA':'OPERACAO',
    'NOITE - CHAPEIRO':'OPERACAO','NOITE - MONTADOR':'OPERACAO','NOITE - FRITADEIRA':'OPERACAO',
    'GABRIEL KHALYL':'PRODUCAO','JOÃO':'PRODUCAO','PABLO':'PRODUCAO',
    'MATEUS':'PRODUCAO','MATEUS PRODUÇÃO':'PRODUCAO','JOSÉ BRUNO':'PRODUCAO',
    'FABIO':'PRODUCAO','SERGIO CAMARAS':'PRODUCAO','WESLEY MOTORISTA':'PRODUCAO','FAXINA':'PRODUCAO',
    'HENRIQUE':'PRODUCAO','PEDRO':'PRODUCAO','DAVI':'PRODUCAO','DINA':'PRODUCAO','SANDRO':'PRODUCAO',
  };
  const dept  = COLLAB_DEPT_RANK[nomeCard] || '';
  const turno = _colabChoiceContext.turno || '';
  const params = new URLSearchParams();
  if (dept)  params.set('dept', dept);
  if (turno && turno !== 'todos') params.set('turno', turno);
  // Passa sessão como _jbs para ranking.html
  try {
    if (typeof JbAuth !== 'undefined' && JbAuth.read) {
      const u = JbAuth.read();
      if (u) {
        const json  = JSON.stringify(u);
        const token = encodeURIComponent(btoa(unescape(encodeURIComponent(json))));
        params.set('_jbs', token);
      }
    }
  } catch(e) {}
  const url = 'ranking.html' + (params.toString() ? '?' + params.toString() : '');
  location.href = url;
}
...
function selectTurno(turno) {
  ...
  if (_isColaboradorRestrito()) {
    const nomeCard = (JB_SESSION.nome_card||'').trim();
    _colabChoiceContext = { nomeCard, turno, dia: S.dia };
    _mostrarEscolhaColaborador(nomeCard, turno);
    return;
  }
  _irParaDept();
}
